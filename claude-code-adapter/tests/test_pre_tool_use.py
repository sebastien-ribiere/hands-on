...
def test_signals_incomplete_without_calling_it_a_deviation(run_hook, attached):
    output = run_hook("pre_tool_use.py", attached)
    hso = output["hookSpecificOutput"]
    assert hso["permissionDecision"] == "allow"
    assert "GOLDEN THREAD -- VERIFICATION INCOMPLETE" in hso["additionalContext"]
    assert "never verified" in hso["additionalContext"]
    assert "leaving the supported path" not in hso["additionalContext"]
    assert "Run: golden-thread verify" in hso["additionalContext"]
...
def test_signals_without_denying_when_off_path(run_hook, attached):
    ...
    assert "You are leaving the supported path." in hso["additionalContext"]
    assert "Missing: ARCH-001" in hso["additionalContext"]
    assert "spells.elements.fire" in hso["additionalContext"]


def test_signals_stale_without_calling_it_a_deviation(run_hook, attached):
    verify(attached)
    ...
    hso = output["hookSpecificOutput"]
    assert hso["permissionDecision"] == "allow"
    assert "GOLDEN THREAD -- EVIDENCE STALE" in hso["additionalContext"]
